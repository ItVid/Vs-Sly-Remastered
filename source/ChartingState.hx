...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var isRoger:Bool = false;

	private var lastNote:Note;
	var claps:Array<Note> = [];
	var speedReminder:FlxText = new FlxText(10, 10, 0, '', 10);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ALT) isRoger = !isRoger;

		if (isRoger) speedToggle = "true" else speedToggle = "false";

		speedReminder.text = "Roger Notes = " + speedToggle;
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, false, daSpeed);
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteSus = 0;
		var noteRoger:Bool = isRoger;

		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.isSpeed]);
		else
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteRoger]);
