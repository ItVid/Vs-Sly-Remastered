...
class GameJoltAPI // Connects to tentools.api.FlxGameJolt
{
    ...
    public static function checkplatinum()
    {
        if(userLogin)
        {
            trace(GJApi.TROPHIES_MISSING);
             if (GJApi.TROPHIES_MISSING == -1)
...
//             }
		...
		var charBop:FlxSprite = new FlxSprite(-300, -180).loadGraphic(Paths.image('gamejolt_menu_sly', 'preload'));
        charBop.setGraphicSize(Std.int(charBop.width * 0.67));
		add(charBop);

        gamejoltText = new FlxText(0, 25, 0, "GameJolt Integration", 16);
        gamejoltText.screenCenter(X);
        gamejoltText.x += baseX;
        gamejoltText.color = FlxColor.fromRGB(138,43,226);
        add(gamejoltText);
        ...
        helpBox.color = FlxColor.fromRGB(138,43,226);
        ...
        #end
        logOutBox.color = FlxColor.RED /*FlxColor.fromRGB(138,43,226)*/ ;
